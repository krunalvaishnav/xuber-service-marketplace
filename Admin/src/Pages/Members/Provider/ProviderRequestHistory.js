...
import styles from "../../../Styles/Members/Provider/ProviderRequestHistory.module.css";
...
function ProviderRequestHistory() {
  ...
  const getApi = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/admin/provider/${id}/request`
      );
      if ($.fn.DataTable.isDataTable("#userTable")) {
        $("#userTable").DataTable().destroy();
      }

      let requestData = response.data.providerRequest;
      ...
      if (response.data.providerRequest.length > 0) {
        setProviderName({
          first_name: response.data.providerRequest[0].provider_first_name,
          last_name: response.data.providerRequest[0].provider_last_name,
        });
      }
    } catch (error) {
              ...
              >
                <thead>
                  <tr>
                    ...
                    <th className="text-left">Cash Collected (₹)</th>
                  ...
                  {data.length > 0 ? (
                    data.map((request, index) => (
                      <tr key={index}>
                        ...
                        <td className="text-left">{request.total || 0}</td>
                        <td className="text-left">
                          {request.provider_earning || 0}
                        </td>
                        <td
                          style={{
                            color:
...
export default ProviderRequestHistory;
