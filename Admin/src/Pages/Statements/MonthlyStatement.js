...
import styles from "../../Styles/Statements/MonthlyStatement.module.css";
...
function MonthlyStatement() {
  ...
  const getMonthlyStatementsCount = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/admin/monthlystatement-count`
      );
      setTimeout(() => {
        setTotalCounts(response.data.monthlyStatementsCount[0]);
      }, 1000);
    } catch (error) {
      console.error(error);
    }
  ...
  const getMonthlyStatementsData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/admin/monthlystatement-data`
      );
      let requestData = response.data.monthlyStatementsData;
  ...
  useEffect(() => {
    getMonthlyStatementsCount();
    getMonthlyStatementsData();
  }, []);
