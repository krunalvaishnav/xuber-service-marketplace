...
import styles from "../../Styles/Statements/DailyStatement.module.css";
...
function DailyStatement() {
  ...
  const getDailyStatementsCount = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/admin/dailystatement-count`
      );
      setTimeout(() => {
        setTotalCounts(response.data.dailyStatementsCount[0]);
      }, 1000);
    } catch (error) {
      console.error(error);
    }
  ...
  const getDailyStatementsData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/admin/dailystatement-data`
      );
      let requestData = response.data.dailyStatementsData;
  ...
  useEffect(() => {
    getDailyStatementsCount();
    getDailyStatementsData();
  }, []);
