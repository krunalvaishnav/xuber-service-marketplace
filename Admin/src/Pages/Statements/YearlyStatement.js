...
import styles from "../../Styles/Statements/YearlyStatement.module.css";
...
function YearlyStatement() {
  const [data, setData] = useState([]);
  const [totalCounts, setTotalCounts] = useState({});
  const [showModal, setShowModal] = useState(false);
  const [selectedRequest, setSelectedRequest] = useState(null);

  const getYearlyStatementsCount = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/admin/yearlystatement-count`
      );
      setTimeout(() => {
        setTotalCounts(response.data.yearlyStatementsCount[0]);
      }, 1000);
    } catch (error) {
      console.error(error);
    }
  ...
  const getYearlyStatementsData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/admin/yearlystatement-data`
      );
      let requestData = response.data.yearlyStatementsData;
  ...
  useEffect(() => {
    getYearlyStatementsCount();
    getYearlyStatementsData();
  }, []);
